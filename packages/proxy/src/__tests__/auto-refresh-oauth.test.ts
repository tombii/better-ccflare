...
describe("AutoRefreshScheduler - OAuth Reauthentication", () => {
	...
	describe("Token expiration detection", () => {
		...
		it("should identify tokens expiring within buffer period", () => {
			...
			const expiringSoonAccount = {
				...
				refresh_token: "refresh-token",
				access_token: "access-token",
				expires_at: now + 20 * 60 * 1000, // Expires in 20 minutes (within 30-minute buffer)
				rate_limit_reset: null,
				custom_endpoint: null,
			};

			const isExpired = (scheduler as any).isTokenExpired(
				expiringSoonAccount,
				now,
			);
			expect(isExpired).toBe(true);
		...
		it("should identify valid tokens", () => {
			...
			const validAccount = {
				...
				refresh_token: "refresh-token",
				access_token: "access-token",
				expires_at: now + 40 * 60 * 1000, // Expires in 40 minutes (outside 30-min buffer)
				rate_limit_reset: null,
				custom_endpoint: null,
			};
	...
	describe("OAuth session management", () => {
		it("should create OAuth session for reauthentication", async () => {
			...
			// Mock the ProxyContext to use port 8082
			(scheduler as any).proxyContext = {
				runtime: {
					port: 8082,
					clientId: "test-client-id",
				},
			...
			const mockConfig = mock(() => ({
				runtime: { clientId: "test-client-id" },
			}));
			const mockDbOps = {
			...
			try {
				...
				const sessions = db
					.query("SELECT * FROM oauth_sessions WHERE account_name = ?")
					.all("reauth-account");
				...
				// Clean up test data
				db.run(
					"DELETE FROM oauth_sessions WHERE account_name = 'reauth-account'",
				);
			} finally {
				// Restore mocks
		...
		it("should handle browser opening failures gracefully", async () => {
			...
			const mockOpen = mock(() =>
				Promise.reject(new Error("Browser not available")),
			);
			...
			const mockImportOAuth = mock(() =>
				Promise.resolve({ getOAuthProvider: () => mockOAuthProvider }),
			);
			...
			try {
				...
				const sessions = db
					.query("SELECT * FROM oauth_sessions WHERE account_name = ?")
					.all("fail-browser-account");
				expect(sessions.length).toBe(1);

				// Clean up test data
				db.run(
					"DELETE FROM oauth_sessions WHERE account_name = 'fail-browser-account'",
				);
			} finally {
				// Restore mocks
		...
		it("should use Windows PowerShell on Windows platform", async () => {
			...
			const mockOpen = mock(() =>
				Promise.reject(new Error("Open package not available")),
			);
			const mockSpawn = mock(() => {
				const mockProcess = {
					unref: mock(() => {}),
			...
			globalThis.import = mock((modulePath: string) => {
				if (modulePath === "open") {
					...
					return Promise.resolve({
						Config: mock(() => ({ runtime: { clientId: "test-client-id" } })),
					});
				} else if (modulePath === "@better-ccflare/database") {
					return Promise.resolve({
						DatabaseOperations: mock(() => ({ createOAuthSession: mock() })),
					});
				} else if (modulePath === "@better-ccflare/oauth-flow") {
					return Promise.resolve({
	...
	describe("Window refresh logic", () => {
		it("should identify accounts needing window refresh", () => {
			...
			const shouldRefresh = (scheduler as any).shouldRefreshWindow(
				accountNeedingRefresh,
				now,
			);
		...
		it("should skip accounts with current reset times", () => {
			...
			(scheduler as any).lastRefreshResetTime.set(
				"test-no-refresh",
				futureTime - 1000,
			);

			const shouldRefresh = (scheduler as any).shouldRefreshWindow(
				accountNotNeedingRefresh,
				now,
			);
	...
	describe("Account selection logic", () => {
      ...
      `)
			...
			// Clean up
			db.run(
				"DELETE FROM accounts WHERE id IN ('expired-account', 'window-account')",
			);
		});
	...
	describe("Failure tracking", () => {
		it("should track consecutive failures", async () => {
			...
			try {
				...
				const failureCount = (scheduler as any).consecutiveFailures.get(
					"test-failures",
				);
		...
		it("should reset failure count on success", async () => {
			...
			const _mockOpen = mock(() => Promise.resolve());
