...
describe("OAuth Callback Handler - Browser Support", () => {
  ...
  describe("GET request handling (browser redirect)", () => {
    it("should handle browser callback with query parameters", async () => {
      ...
      const originalCreateOAuthFlow = createOAuthFlow;

      // Mock the module import
      const originalImport = globalThis.import;
      globalThis.import = mock(() => Promise.resolve({ createOAuthFlow: mockCreateOAuthFlow }));

      try {
        const url = new URL(`http://localhost/oauth/callback?state=${sessionId}&code=${code}`);
    ...
    it("should handle GET request with sessionId parameter", async () => {
      ...
      try {
        const url = new URL(`http://localhost/oauth/callback?sessionId=${sessionId}&code=${code}`);
        const request = new Request(url, { method: "GET" });
    ...
    it("should handle GET request without URL parameter", async () => {
      const request = new Request("http://localhost/oauth/callback?state=test&code=test", {
        method: "GET"
      });
  ...
  describe("Error handling", () => {
    it("should handle expired OAuth sessions", async () => {
      const url = new URL("http://localhost/oauth/callback?state=expired-session&code=test-code");
      const request = new Request(url, { method: "GET" });
    ...
    it("should handle OAuth completion errors", async () => {
      ...
      const mockComplete = mock(() => Promise.reject(new Error("OAuth completion failed")));
    ...
    it("should handle database errors gracefully", async () => {
      ...
      try {
        const url = new URL("http://localhost/oauth/callback?state=test-session&code=test-code");
        const request = new Request(url, { method: "GET" });
