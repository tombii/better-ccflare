...
async function addAccount(
	...
	const existingRow = db
		...
		.get(name);
	const existing = existingRow ? toAccount(existingRow) : null;
	if (existing) {
...
function listAccounts() {
	const rows = db
		.query<AccountRow, []>(`SELECT * FROM accounts ORDER BY created_at DESC`)
		.all();
...
Examples:
  bun cli.ts add personal                           Add account (interactive mode)
  bun cli.ts add work --mode max                    Add Max account (will ask for tier)
  bun cli.ts add pro --mode console                 Add API account
  bun cli.ts add premium --mode max --tier 5        Add Max 5x account
  bun cli.ts add enterprise --mode max --tier 20    Add Max 20x account
