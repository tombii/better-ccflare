...
| `session-drain-soonest` | Same session semantics as `session` (5h windows, auto-fallback, session stickiness), but prefers the account whose weekly_all usage window resets soonest, so weekly capacity is drained before it expires ("use it or lose it"). Accounts without weekly telemetry rank last; ties fall back to priority, then utilization. Switches accounts only at session boundaries or when another account's weekly reset is strictly earlier (60s cohort tolerance) | Multi-account pools with staggered weekly resets where unused weekly capacity should be consumed before it is lost |
...
| `MODEL_SCOPED_CAPACITY_ROUTING` | `off` leaves account selection unchanged. `exhausted` skips an account for a request when its weekly per-model-family cap (`limits[] kind=weekly_scoped`, e.g. a Fable/Opus/Sonnet-specific quota) is at/above 100% with a future reset — both in normal and combo-slot routing. When every candidate account for a model family is filtered out this way, the request gets a structured `529 model_family_exhausted` response instead of exhausting the per-account failover loop against accounts already known to reject that family. Same as the `model_scoped_capacity_routing` config file field; env var takes precedence | `off` | `MODEL_SCOPED_CAPACITY_ROUTING=exhausted` |
