...
| `CCFLARE_CODEX_SYNTHETIC_COUNT_TOKENS` | Set to `0` to make Codex's synthetic `count_tokens` endpoint return a typed error instead of a character-based estimate, so gateways don't mistake the estimate for a real upstream count. Default: enabled (estimate) | `1` | `CCFLARE_CODEX_SYNTHETIC_COUNT_TOKENS=0` |
