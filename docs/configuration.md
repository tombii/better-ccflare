...
| `CODEX_CLAUDE_OAUTH_MODE` | Controls whether the OpenAI Responses adapter may route Codex-style `/v1/responses` traffic to Anthropic OAuth accounts. Default safe behavior excludes Anthropic OAuth. Set to `claude-code-compat` only when better-ccflare should own Claude-Code-compatible request shaping for an allowlisted OpenCodex lane | `safe` | `CODEX_CLAUDE_OAUTH_MODE=claude-code-compat` |
| `CODEX_CLAUDE_OAUTH_ACCOUNT_ALLOWLIST` | Comma-separated Anthropic OAuth account names or IDs eligible for `CODEX_CLAUDE_OAUTH_MODE=claude-code-compat`. Empty or missing allowlist forces safe mode, even when compat mode is requested | unset | `CODEX_CLAUDE_OAUTH_ACCOUNT_ALLOWLIST=Jenny_claude,Sandra_Claude` |
