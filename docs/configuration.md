...
| `CCFLARE_CODEX_PROMPT_CACHE_KEY` | Opt-in: attach an OpenAI `prompt_cache_key` to converted Codex requests, per [OpenAI's prompt-caching guidance](https://platform.openai.com/docs/guides/prompt-caching) for GPT-5.6-family models. Only applies when the account's resolved endpoint is OpenAI's own `chatgpt.com` / `api.openai.com` — custom/self-hosted OpenAI-compatible endpoints and native Anthropic accounts are unaffected regardless of this setting | - (off) | `CCFLARE_CODEX_PROMPT_CACHE_KEY=1` |
