...
| `BETTER_CCFLARE_AUTO_VACUUM` | Set to `false` to disable the unattended `PRAGMA incremental_vacuum` reclaim backstop (both the hourly retention-driven tick and the 5-minute catch-up tick that runs when the freelist grows large — see [Database Configuration](#database-configuration)). Retention deletion and payload cleanup are unaffected; only the reclaim step is skipped, checked on every tick so flipping it takes effect without a restart. Useful during an external backup, a manual `bun run cli --compact`, or another maintenance window where you want zero automatic writer-slot contention from reclaim. SQLite only — no-op on PostgreSQL, which has no freelist/incremental-vacuum concept. Same as the `auto_vacuum_enabled` config file field; env var takes precedence | `true` | `BETTER_CCFLARE_AUTO_VACUUM=false` |
...
Both ticks are bounded, chunked, and yield between chunks so a large reclaim never holds up proxy requests for long. Set `BETTER_CCFLARE_AUTO_VACUUM=false` to disable both entirely (see the environment variable table above) — retention deletion and payload cleanup keep running either way, only the reclaim step is skipped. No-op on PostgreSQL, which has no freelist or incremental-vacuum concept.
