...
| `CCFLARE_SERVER_ERROR_RETRY_ENABLED` | Retry a transient upstream server error (HTTP 500/502/503/504, any provider) once in place, then bench the account briefly and fail over to the next one. Set to `false` to forward these responses straight to the client again. The attempt count and backoff are not separate knobs — they reuse `CCFLARE_OVERLOAD_RETRY_MAX_ATTEMPTS` / `_BASE_MS` / `_MAX_MS` above, since both paths are "re-issue the same request once because the upstream is transiently unwell". The retry replays the same request body (Anthropic's Messages API has no idempotency key); this matches what the official Anthropic SDKs do on 5xx by default and what the proxy already does on 529 and on account failover. Set to `false` to keep only the bench + failover. A response carrying `x-should-retry: false` skips the retry and goes straight to the bench; synthetic keepalive/auto-refresh probes are not retried or benched at all | `true` | `CCFLARE_SERVER_ERROR_RETRY_ENABLED=false` |
