...
| `BETTER_CCFLARE_SHOW_COMBOS` | **Legacy.** Combos are now switched on and off in the dashboard's Combos tab, and that setting gates routing rather than only hiding the tab — previously a combo hidden from the UI kept steering traffic. This variable is adopted into the setting once, at boot, when the config field is absent, and is never consulted again. An install that already has combos but never set it adopts *enabled*, so upgrading does not silently stop routing them | adopted once, then ignored | `BETTER_CCFLARE_SHOW_COMBOS=true` |
