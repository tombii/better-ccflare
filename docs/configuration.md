...
| `BETTER_CCFLARE_SHOW_COMBOS` | Whether combos take part in routing, and whether the dashboard shows the Combos tab. Previously this gated the tab only, while the proxy kept routing on whatever combos the database held — a combo hidden from the UI still steered traffic. It now sets the same switch the Combos tab exposes, and takes precedence over it (the switch is shown locked while this is set). An install that already had combos is seeded to enabled on first boot, so upgrading never changes routing silently. Same as the `combos_enabled` config file field | `false` | `BETTER_CCFLARE_SHOW_COMBOS=true` |
