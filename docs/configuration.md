...
|-------|------|---------|-------------|
| `lb_strategy` | string | `"session"` | Load balancing strategy. Recommended options: `"session"` (default), `"session-drain-soonest"` (same session semantics, prefers the soonest-resetting weekly window), `"session-drain-soonest-strict"` (drain ranking applies to every selection; an active 5h session only breaks ties), or `"session-affinity"` (per-client-session stickiness — see below for when this is a better fit than `"session"`). Per-*request* spreading strategies (e.g. `"least-used"`) risk account bans — see warning below |
...
⚠️ **WARNING**: Prefer session-class strategies — `session` (default), `session-drain-soonest`, `session-drain-soonest-strict`, or `session-affinity` — which all preserve session-length stickiness to an account (5-hour windows, minimal switching). Strategies that spread *individual requests* across accounts regardless of session (e.g. `least-used`) create unnatural per-message account-hopping and can trigger Claude's anti-abuse systems — avoid those unless you understand the risk.

| Strategy | Description | Use Case |
|----------|-------------|----------|
| `session` | Tracks a single global "most recently active" session and pins ALL traffic to that one account until it rate-limits or the session expires, then rotates to the next | Default and recommended for a single Claude Code project/session at a time — mimics natural usage patterns and optimizes resource utilization |
| `session-affinity` | Keys stickiness on the *client's own* session id (Claude Code's `metadata.user_id`) instead of one global session slot: each concurrent client-session is assigned its own least-loaded account and stays pinned to it for the session window, while still only switching accounts once per session (not per request) | Running multiple Claude Code projects/sessions concurrently against a multi-account pool — avoids the cache-thrash where a newer session's client displaces an older one's on the single shared `session` slot ([#240](https://github.com/tombii/better-ccflare/issues/240)). If a client never sends `metadata.user_id`, that traffic degrades to per-request least-used routing with no stickiness — confirm your client sends it before relying on this |
...
`session-affinity` is not yet the default and must be opted into via `LB_STRATEGY=session-affinity` or `"lb_strategy": "session-affinity"` — it is deliberately not exposed in the dashboard's strategy selector, which only offers pure session-class options (see the dashboard's `RoutingCard` source for the current reasoning). If you run several concurrent Claude Code sessions against one account pool, it is worth trying; if you've run it in production, real-world feedback on account-ban rates is welcome on [#240](https://github.com/tombii/better-ccflare/issues/240).
...
| `BETTER_CCFLARE_AUTO_VACUUM` | Set to `false` to disable the unattended `PRAGMA incremental_vacuum` reclaim backstop (both the hourly retention-driven tick and the 5-minute catch-up tick that runs when the freelist grows large — see [Database Configuration](#database-configuration)). Retention deletion and payload cleanup are unaffected; only the reclaim step is skipped. **Read once at process start — changing the env var or the config file value takes effect only after a restart.** Only the literal values `false` and `0` disable it; any other value (including `off`, `no`, or `disabled`) is treated as enabled. Useful during an external backup, a manual `bun run cli --compact`, or another maintenance window where you want zero automatic writer-slot contention from reclaim — set it and restart *before* the window starts. SQLite only — no-op on PostgreSQL, which has no freelist/incremental-vacuum concept. Same as the `auto_vacuum_enabled` config file field; env var takes precedence | `true` | `BETTER_CCFLARE_AUTO_VACUUM=false` |
